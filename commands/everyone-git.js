...
return message.reply("Ses kanalında olman lazım!").sil(3);
}
...
if (!kullanıcı) return message.channel.send('Bir Kullanıcı Belirt.').sil(3);

let rol = message.mentions.roles.first();
let member = message.guild.member(kullanıcı);

if (!member.voice.channel) return message.channel.send('Etiketlenen Kullanıcı Ses Kanalında Değil.').sil(3);
...
message.channel.send(kabul).sil(10);
...
message.channel.send(striga).sil(10);
}
})}
