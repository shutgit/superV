...
const uyarılar = new JsonDatabase({ databasePath: './databases/Warnings.json' });
const sicil = new JsonDatabase({ databasePath:'./databases/Sicil.json'});

exports.run = async(client, message, args) => { 

if(!message.member.roles.cache.has(config.roller.warnStaff) && !message.member.hasPermission("ADMINISTRATOR")) return message.react(config.emoji.no);
...
uyarılar.push(`uyarı.${member.id}`, { Staff: message.author.id, Reason: sebep, Date: Date.now() });
sicil.push(`sicil.${member.id}`, { Type: "WARN", Staff: message.author.id, Reason: sebep, Date: Date.now() });
