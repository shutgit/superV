...
exports.run = async(client, message, args) => {
    ...
    let embed = new MessageEmbed().setColor('RANDOM').setTimestamp().setFooter(`Arawn`)
    let data = await staff.get(`ekip.${message.guild.id}`) || []
    let rol = message.mentions.roles.first() || message.guild.roles.cache.get(args[0])
    let enaltyt = message.guild.roles.cache.get(ayarlar.roller.registerStaff)
    let tag = "31"
    if (args[0] === "ekle") {
        ...
        staff.push(`ekip.${message.guild.id}`, hedefRol.id, )
    ...
    if (args[0] === "sil") {
        ...
        if (data.length === 1) {
            staff.delete(`ekip.${message.guild.id}`)
            message.channel.send(embed.setDescription(`${hedefRol} Rolü ekip sisteminden kaldırıldı!`))
        } else {
            staff.set(`ekip.${message.guild.id}`, data.filter(s => s !== hedefRol.id))
            message.channel.send(embed.setDescription(`${hedefRol} Rolü ekip sisteminden kaldırıldı.`))
        }
    }
    if (args[0] === 'say') {
